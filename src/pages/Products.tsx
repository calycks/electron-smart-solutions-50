...
const Products = () => {
  ...
  const categories = [
    { name: "Security Systems", count: 2, icon: Camera },
    { name: "Communication", count: 2, icon: Phone },
    { name: "Display Technology", count: 1, icon: Monitor },
    { name: "Smart Controls", count: 1, icon: Lightbulb }
  ];
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {categories.map((category, index) => {
              ...
              return (
                <div 
                  key={category.name}
                  className={`bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 fade-in-up ${isVisible ? 'animate' : ''}`}
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            {products.map((product, index) => {
              ...
              return (
                <div 
                  key={product.title}
                  className={`bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 fade-in-up ${isVisible ? 'animate' : ''}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div 
                key={item.title}
                className={`text-center bg-white rounded-2xl p-8 shadow-lg fade-in-up ${isVisible ? 'animate' : ''}`}
                style={{ animationDelay: `${index * 200}ms` }}
              >
